  ...
  const renderRow = (category: Category, isSub = false) => {
    return (
      <tr 
        ...
        className={`border-b border-zinc-800/50 hover:bg-zinc-800/25 transition-all duration-200 cursor-move ${
          isSub ? 'bg-zinc-950/20 font-medium' : 'bg-zinc-900/10 font-bold'
        }`}
      >
        <td className="p-4">
          <div className={`flex items-center gap-2 ${isSub ? 'pl-8' : ''}`}>
            {isSub && <CornerDownRight className="w-4 h-4 text-zinc-500 flex-shrink-0" />}
            <div className="w-16 h-12 bg-zinc-800 border border-zinc-700 overflow-hidden rounded flex-shrink-0">
              {category.image_url ? (
                <img src={category.image_url} alt={category.name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-zinc-600 text-[10px] font-bold uppercase font-mono">No Img</div>
              )}
            </div>
          </div>
        </td>
        <td className="p-4">
          <span className={`text-sm text-brand-offwhite ${isSub ? 'text-zinc-300' : 'text-brand-offwhite'}`}>
            {category.name}
          </span>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] text-zinc-500 font-mono">/{category.slug}</span>
            {category.description && (
              <span className="text-[10px] text-zinc-650 truncate max-w-xs">— {category.description}</span>
            )}
          </div>
        </td>
        <td className="p-4">
